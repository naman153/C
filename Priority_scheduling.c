...
int rearrange(int n,float *AT1, float *BT1, float *ID1, float *P1){ 
    int time;

    for(int i=1;i<n;i++){           ///////////////////////////Finding first arriving process.//////////////////////////////
        if(AT1[i]<AT1[0]){
            int temp= BT1[i];
            BT1[i] = BT1[0];
            BT1[0] = temp;
...
int rearrange_ans(int n,float *AT1, float *BT1, float *ID1, float *CT1, float *TAT1, float *WT1, float *P1){       
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            if(ID1[i] > ID1[j]){
                int temp= ID1[i];
...
int main(){
    ...
    scanf("%d", &n);
    float ID[n], AT[n], BT[n], CT[n], TA[n], WT[n], P[n]; // (AT=Arrival Time, BT= Burst Time, CT= Completion Time, TA= Turnaround Time, WT= Worst TIme)
    ...
    for(int i=0;i<n;i++){
        ...
        scanf("%f",&BT[i]);
        printf("Enter the priority of process %d: ", i);    //Input Priority of Processes.
        scanf("%f",&P[i]);
    }
    ...
    printf("|============|==============|===========|===========|\n");
    for(int i=0;i<n;i++){
    printf("|P%-6.0f     |%6.2f        |%6.2f     |%6.2f     |\n", ID[i], AT[i], BT[i],P[i]);
    printf("|------------|--------------|-----------|-----------|\n");
    }


    rearrange(n, AT, BT, ID,P);   //reaaranging in terms of arrival time of process.
    ...
    rearrange_ans(n, AT, BT, ID, CT, TA, WT, P);   //reaaranging in terms of process id.
    ...
    printf("|==============|=================|===============|===============|=================|=================|=============|\n");
    for(int i=0;i<n;i++){
    printf("|P%-6.0f       |%6.2f           |%6.2f         |%6.2f         |%6.2f           |%6.2f           |%6.2f       |\n", ID[i],AT[i], BT[i],P[i],CT[i], TA[i], WT[i]);
    printf("|--------------|-----------------|---------------|---------------|-----------------|-----------------|-------------|\n");
    }
